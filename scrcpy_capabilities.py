...
FEATURES: tuple[ScrcpyFeature, ...] = (
    ScrcpyFeature(
        "video",
        "streaming",
        "Mirror the device display with configurable size, FPS, bitrate and codec.",
        "Use for human watch mode and visual debugging.",
        ("--max-size", "--max-fps", "--video-bit-rate", "--video-codec"),
    ),
    ScrcpyFeature(
        "audio",
        "streaming",
        "Forward device audio, microphone, or playback streams where Android supports it.",
        "Use when the human operator needs audible feedback.",
        ("--audio-source", "--audio-codec", "--audio-buffer"),
    ),
    ScrcpyFeature(
        "recording",
        "capture",
        "Record video or audio to files such as MP4, MKV, OPUS, FLAC or WAV.",
        "Use to create reproducible traces of an agent session.",
        ("--record", "--record-format"),
    ),
    ScrcpyFeature(
        "virtual_display",
        "display",
        "Create an isolated Android virtual display, optionally flex-resized to the scrcpy window.",
        "Use for app-isolated workflows that should not disturb the main device screen.",
        ("--new-display", "--flex-display", "--start-app"),
    ),
    ScrcpyFeature(
        "camera",
        "capture",
        "Mirror or record Android cameras on Android 12+.",
        "Use for camera inspection or camera recording workflows.",
        ("--video-source", "--camera-id", "--camera-facing", "--camera-size"),
    ),
    ScrcpyFeature(
        "hid_input",
        "control",
        "Simulate physical keyboard, mouse, or gamepad input through UHID/AOA modes.",
        "Use when ADB input is insufficient and the task benefits from real HID semantics.",
        ("--keyboard", "--mouse", "--gamepad"),
    ),
    ScrcpyFeature(
        "otg",
        "control",
        "Control only through USB OTG-style keyboard and mouse without ADB debugging.",
        "Use for recovery/control cases where mirroring is unavailable.",
        ("--otg", "--keyboard", "--mouse"),
    ),
    ScrcpyFeature(
        "clipboard",
        "control",
        "Synchronize or inject clipboard content through scrcpy shortcuts and paste modes.",
        "Use with human oversight for text transfer.",
        ("--legacy-paste", "--no-clipboard-autosync"),
    ),
    ScrcpyFeature(
        "diagnostics",
        "inspection",
        "List apps, displays, cameras, camera sizes and encoders from the target device.",
        "Use before choosing a recipe or device-specific flag.",
        ("--list-apps", "--list-displays", "--list-cameras", "--list-camera-sizes", "--list-encoders"),
    ),
    ScrcpyFeature(
        "window",
        "desktop",
        "Control scrcpy window placement, full screen, top-most behavior and decoration.",
        "Use to create reliable human observation layouts.",
        ("--fullscreen", "--always-on-top", "--window-title", "--window-width", "--window-height"),
    ),
)
...
SHORTCUTS: tuple[ScrcpyShortcut, ...] = (
    ...
    ScrcpyShortcut(
        ...
        ("MOD+h", "Middle-click"),
        "android_navigation",
        agent_use="ADB equivalent: input keyevent KEYCODE_HOME.",
    ),
    ScrcpyShortcut(
        "back",
        ("MOD+b", "MOD+Backspace", "Right-click"),
        "android_navigation",
        agent_use="ADB equivalent: input keyevent KEYCODE_BACK.",
    ),
    ScrcpyShortcut(
        "app_switch",
        ("MOD+s", "4th-click"),
        "android_navigation",
        agent_use="ADB equivalent: input keyevent KEYCODE_APP_SWITCH.",
    ),
    ScrcpyShortcut("menu", ("MOD+m",), "android_navigation"),
    ScrcpyShortcut(
        "volume_up", ("MOD+Up",), "android_navigation", agent_use="ADB equivalent: input keyevent KEYCODE_VOLUME_UP."
    ),
    ScrcpyShortcut(
        "volume_down",
        ("MOD+Down",),
        "android_navigation",
        agent_use="ADB equivalent: input keyevent KEYCODE_VOLUME_DOWN.",
    ),
    ScrcpyShortcut(
        "power", ("MOD+p",), "android_navigation", agent_use="ADB equivalent: input keyevent KEYCODE_POWER."
    ),
    ...
    ScrcpyShortcut(
        ...
        ("MOD+k",),
        "hid",
        agent_use="ADB equivalent: am start -a android.settings.HARD_KEYBOARD_SETTINGS.",
    ),
    ScrcpyShortcut(
        "toggle_fps_counter", ("MOD+i",), "diagnostics", agent_use="Equivalent launch flag: --print-fps starts enabled."
    ),
    ...
    ScrcpyShortcut(
        ...
        ("Drag & drop APK",),
        "file_transfer",
        agent_use="Prefer explicit human approval before installs.",
    ),
    ScrcpyShortcut(
        "push_file",
        ("Drag & drop non-APK file",),
        "file_transfer",
        agent_use="Prefer adb push or explicit human approval.",
    ),
...
OPTIONS: tuple[ScrcpyOption, ...] = (
    ...
    ScrcpyOption(
        ...
        ("--record=session.mp4",),
        "May capture sensitive screen content.",
    ),
    ScrcpyOption(
        "--record-format", "choice", "recording", "Force recording container/format.", ("--record-format=mp4",)
    ),
    ScrcpyOption(
        "--new-display",
        "optional:WIDTHxHEIGHT/DPI",
        "virtual_display",
        "Create an Android virtual display.",
        ("--new-display=1920x1080/420", "--new-display"),
    ),
    ScrcpyOption(
        "--flex-display",
        "bool",
        "virtual_display",
        "Resize virtual display to match the scrcpy window.",
        ("--flex-display",),
    ),
    ScrcpyOption(
        "--start-app",
        "package",
        "app",
        "Start an Android app by package or fuzzy name.",
        ("--start-app=org.videolan.vlc", "--start-app=+?firefox"),
    ),
    ScrcpyOption(
        "--keyboard", "choice:disabled|sdk|uhid|aoa", "hid", "Select keyboard input mode.", ("--keyboard=uhid",)
    ),
    ...
    ScrcpyOption(
        ...
        "mod-list",
        "shortcuts",
        "Customize the shortcut modifier.",
        ("--shortcut-mod=rctrl", "--shortcut-mod=lctrl,lsuper"),
    ),
    ScrcpyOption(
        "--otg",
        "bool",
        "otg",
        "Run OTG control mode over USB.",
        ("--otg",),
        "Control-only mode; may capture keyboard/mouse.",
    ),
    ...
    ScrcpyOption(
        "--always-on-top", "bool", "window", "Keep the scrcpy window above other windows.", ("--always-on-top",)
    ),
    ScrcpyOption(
        "--turn-screen-off",
        "bool",
        "device",
        "Turn device screen off immediately while mirroring.",
        ("--turn-screen-off",),
    ),
    ScrcpyOption(
        "--stay-awake",
        "bool",
        "device",
        "Keep the device awake while plugged in during the session.",
        ("--stay-awake",),
    ),
    ...
    ScrcpyOption(
        "--camera-facing",
        "choice:front|back|external",
        "camera",
        "Select camera by facing direction.",
        ("--camera-facing=front",),
    ),
    ScrcpyOption(
        "--camera-size", "WIDTHxHEIGHT", "camera", "Select explicit camera size.", ("--camera-size=1920x1080",)
    ),
    ...
    ScrcpyOption(
        "--list-camera-sizes", "bool", "diagnostics", "List valid camera capture sizes.", ("--list-camera-sizes",)
    ),
    ScrcpyOption("--list-displays", "bool", "diagnostics", "List device displays.", ("--list-displays",)),
    ScrcpyOption(
        "--list-encoders", "bool", "diagnostics", "List available video and audio encoders.", ("--list-encoders",)
    ),
    ScrcpyOption(
        "--v4l2-sink", "path", "linux", "Expose video as a V4L2 sink.", ("--v4l2-sink=/dev/video2",), platform="linux"
    ),
)
...
RECIPES: tuple[ScrcpyRecipe, ...] = (
    ScrcpyRecipe(
        "high-quality-mirror",
        "High-quality mirror",
        "H.265 1080p/60 mirror with audio disabled for responsive human watch mode.",
        ("--video-codec=h265", "--max-size=1920", "--max-fps=60", "--no-audio"),
        "Use when a human needs a sharp low-latency observation window.",
    ),
    ScrcpyRecipe(
        "read-only-watch",
        "Read-only watch",
        "Mirror without sending input controls.",
        ("--no-control", "--stay-awake"),
        "Use beside ADB/CUA sessions so humans can watch without accidental clicks.",
    ),
    ScrcpyRecipe(
        "virtual-app",
        "Virtual display app",
        "Start one app in a virtual display.",
        ("--new-display", "--flex-display", "--start-app={package}"),
        "Use for isolated app workflows.",
        requires_package=True,
    ),
    ScrcpyRecipe(
        "camera-recording",
        "Camera recording",
        "Record a camera stream to an MP4 file.",
        ("--video-source=camera", "--video-codec=h265", "--camera-size=1920x1080", "--record={record_file}"),
        "Use for camera inspection or reproducible camera traces.",
        requires_record_file=True,
    ),
    ScrcpyRecipe(
        "hid-input",
        "HID keyboard and mouse",
        "Mirror with UHID keyboard and mouse.",
        ("--keyboard=uhid", "--mouse=uhid"),
        "Use when HID input semantics are more reliable than ADB input.",
    ),
    ScrcpyRecipe(
        "otg-control",
        "OTG control",
        "Control-only OTG mode over USB.",
        ("--otg",),
        "Use for USB control recovery when ADB mirroring is not available.",
    ),
)
...
def get_scrcpy_version(scrcpy_exe: Path) -> dict[str, str]:
    ...
    first_line = (
        (completed.stdout or completed.stderr or "").splitlines()[0] if (completed.stdout or completed.stderr) else ""
    )
...
def recommend_recipe(
    ...
    *,
    ...
    args = [arg.format(package=package or "", record_file=record_file or "") for arg in recipe.args]
